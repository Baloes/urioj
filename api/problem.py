...
class Problem:
    ...
    def __init__(self, problem=None):
        ...
        self.sort_func = None
        self.display_n = len(self.problem)

    def _set_total_submission(self):
        for _id, info in self.problem.items():
            total_submission = int(info['ac']) + int(info['ce']) + int(info['rte']) \
                             + int(info['tle']) + int(info['pe']) + int(info['wa']) \
                             + int(info['pre'])
    ...
    def __repr__(self):
        ...
        for key, value in sorted(self.problem.items(), key=self.sort_func)[:self.display_n]:
